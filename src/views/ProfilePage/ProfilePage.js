...
import { expectIdInReactComponent } from './../../util/expect.js'
...
class ProfilePage extends React.Component {
  ...
  componentWillMount(){
   ...
   //console.log(this.state)
    this.expect=expectIdInReactComponent(window.location.href)
  ...
  componentDidMount() {
    ...
    if (this.expect!==undefined) {
      ...
      const url = `https://patarts-backend.herokuapp.com/api/artist/${this.expect}/show`;
      FETCH("GET",url,header,null)
      .then(fetchStatusHandler)
      .then(resp=>resp.json())
      .then(data=>{
        console.log("result of artist profile by id api")
        console.log(data)
        actions.SAVE_PROFILE_BY_ID_DATA_METHOD({profileData:data})
  ...
  render() {
    ...
    const imageData=this.state.profileData!==undefined?this.state.profileData.data.profile_pic:null
    const imageName=this.state.profileData!==undefined?`${this.state.profileData.data.first_name} ${this.state.profileData.data.last_name}`:""
    return (
      <div>
        <Parallax
            ...
            <GridContainer justify="center">
              <GridItem xs={12} sm={12} md={6}>
                <div className={classes.profile}>
                  <div>
                    <img src={imageData} alt="..." className={imageClasses} />
                  </div>
                  <div className={classes.name}>
                    <h3 className={classes.title}>{imageName}</h3>
                    <h6>DESIGNER</h6>
                    <Button
                      justIcon
                      simple
